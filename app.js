// ============================================
// VPN CONFIG MANAGER - RAILWAY VERSION
...
<body class="min-h-screen py-4 md:py-8 px-3 md:px-6 relative transition-colors duration-300">
    ...
    <script>
        ...
        // Konfigurasi - sekarang pake domain sendiri
        const uuid = atob('M2IwMWE3NzctNTVlNy00OWY2LTg2MzctZDk0ZWU2OTYwN2M2');
        ...
        const SS_LBL = atob('dW5kZWZpbmVk');
        ...
        async function checkProxyStatus(ip, port) {
            ...
            try {
                ...
                // Format baru: proxyip true = ACTIVE, false = INACTIVE
        ...
        function getStatusHtml(proxy) {
            if (!proxy.status) {
                return '<div class="status-badge status-checking"><i class="fas fa-spinner fa-pulse"></i><span>Checking...</span></div>';
            }
            
            const isActive = proxy.status === 'ACTIVE';
            const checkInfo = proxy.checkInfo || {};
            const tooltipText = 'Delay: ' + proxy.delay + ' | Speed: ' + proxy.speed + 
                              (checkInfo.proxyip ? ' | IP: ' + checkInfo.proxyip : '') +
                              (checkInfo.hostname ? ' | Host: ' + checkInfo.hostname : '');
            
            if (isActive) {
                return '<div class="status-badge status-active tooltip" data-tooltip="' + tooltipText + '"><i class="fas fa-check-circle animate-pulse text-green-500"></i><span class="animate-pulse text-green-500 font-semibold">ACTIVE</span></div>';
            } else if (proxy.status === 'ERROR') {
                return '<div class="status-badge status-inactive tooltip" data-tooltip="Connection failed"><i class="fas fa-exclamation-triangle"></i><span>ERROR</span></div>';
            } else {
        ...
        function renderTable() {
            ...
            paged.forEach((proxy, idx) => {
                ...
                tbody.innerHTML += '<tr class="border-b border-white/5 hover:bg-white/5 transition-all">' +
                    '<td class="py-4 px-4"><div class="flex items-center gap-3"><span class="text-2xl">' + getFlagEmoji(proxy.countryCode) + '</span><div><div class="font-bold text-sm md:text-base">' + proxy.country + '</div><div class="text-[11px] text-slate-400 font-mono">' + proxy.ip + ':' + proxy.port + 
                    (checkInfo.proxyip ? '<br><span class="text-[10px] text-cyan-400">→ ' + checkInfo.proxyip + '</span>' : '') + 
                    '</div></div></div></td>' +
                    '<td class="py-4 px-4"><div class="text-sm">' + (proxy.isp || '-') + 
                    (checkInfo.colo ? '<br><span class="text-[10px] text-slate-400">Colo: ' + checkInfo.colo + '</span>' : '') + 
                    '</div></td>' +
...
async function fetchProxyList() {
    const now = Date.now();
    if (cachedProxyList && (now - cacheTime) < CACHE_TTL) {
...
async function getProxyFromPath(pathname) {
    ...
    if (ipPortMatch) {
        const proxy = ipPortMatch[1] + ':' + ipPortMatch[2];
        return proxy;
    }
...
function sha256(message) {
    ...
    for (let i = 0; i < padded.length; i += 64) {
        ...
        for (let t = 0; t < 64; t++) {
            const S1 = rotr(e, 6) ^ rotr(e, 25) ^ rotr(e, 14);
            const ch = (e & f) ^ (~e & g);
        ...
        try {
            ...
            if (isDNS) {
                ...
                if (ws.readyState === WebSocket.OPEN) {
                    ...
                    if (isHeaderSent) {
                        ws.send(Buffer.concat([Buffer.from(udpSizeBuffer), Buffer.from(dnsQueryResult)]));
                    } else {
                        ws.send(Buffer.concat([
                            protocolHeader.version ? Buffer.from(protocolHeader.version) : Buffer.alloc(0),
                            Buffer.from(udpSizeBuffer),
                            Buffer.from(dnsQueryResult)
                        ]));
                        isHeaderSent = true;
                    }
                }
                
                // Setup transform stream for subsequent DNS queries
                udpStreamWrite = async (chunk) => {
                    const resp = await fetch("https://1.1.1.1/dns-query", {
                        method: "POST",
                        headers: { "content-type": "application/dns-message" },
                        body: chunk
            ...
            const targetHost = (prxIP ? prxIP.split(/[:=-]/)[0] : protocolHeader.addressRemote);
            const targetPort = parseInt(prxIP ? prxIP.split(/[:=-]/)[1] : protocolHeader.portRemote);
            
            const tcpSocket = net.createConnection({ host: targetHost, port: targetPort }, () => {
                log('connected to ' + targetHost + ':' + targetPort);
                
                // Send initial data
                tcpSocket.write(Buffer.from(protocolHeader.rawClientData));
                
                // Send response header if exists
                if (protocolHeader.version && ws.readyState === WebSocket.OPEN) {
                    ws.send(Buffer.from(protocolHeader.version));
                    isHeaderSent = true;
                }
                
                remoteSocketWrapper.value = tcpSocket;
                
                // Pipe TCP data to WebSocket
                ...
                tcpSocket.on('close', () => {
                    log('tcpSocket closed');
                    safeCloseWebSocket(ws);
                });
                
                tcpSocket.on('error', (err) => {
                    log('tcpSocket error: ' + err.message);
                    safeCloseWebSocket(ws);
                });
            });
            
            tcpSocket.on('error', (err) => {
                log('tcpSocket connection error: ' + err.message);
                safeCloseWebSocket(ws);
            });
...
server.listen(PORT, () => {
    console.log(`VPN Config Manager running on port ${PORT}`);
});
